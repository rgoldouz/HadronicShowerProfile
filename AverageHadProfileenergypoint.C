...
double Gamma1=0;
Float_t xx =x[0];
Gamma1 = (par[1])*pow(xx*(par[1]),par[0]-1.0)*exp(-xx*(par[1]))/tgamma(par[0]);
return Gamma1;
}
*/
...
void AverageHadProfileenergypoint(){
...
if (energies[idx] < energies[num]) continue;
...
//        if (HADshapeIL->Integral() < energies[num] - energies[num]/50 || HADshapeIL->Integral()> energies[num] + energies[num]/50) continue;
//        if (h_H->Integral() < energies[num] - energies[num]/50 || h_H->Integral()> energies[num] + energies[num]/50) continue;
        if (HADshapeIL->Integral() <   energies[idx]/5) continue;      
