 ...
 *
 * In this code I am going to regenerate the alpha beta using random generator considering their correlation.
 *   Reza Goldouzian, 8 Feb  2016
 **********************************************************************************/
...
void alphabeta_regenerate(){

double meanalpha = -0.2729;
double meanbeta = 0.1722;
double varalpha = 0.9595;
double varbeta = 0.7742;
double corr = 0.6296;
//double corr = 0.37;
...
abcorr_H = new TH2F("abcorr_H","abcorr_H",100,-10,10,100,-10,10);
...
abcorr_H->Fill(alpha,beta);

Eprofile = new TH1F("Eprofile","Eprofile",30,0,15);
funcH = new TF1("GammaH",GammaProfile,0  , 100,nGammaPar);
funcH->SetParameters(exp(alpha) ,exp(beta));       
for (int j = 0; j < 10000; ++j){
//Eprofile->Fill(funcH->GetRandom(),1/10000);
...
hists[100-1]->Scale(1/hists[100-1]->Integral());
hists[100-1]->Draw();
//Eprofile->Draw();

/*
g2Random = new TRandom3();
   ...
   for(unsigned int idx=0; idx<samples_.size(); ++idx){
	...
	abcorr_H = new TH2F("abcorr_H","abcorr_H",100,0,10,100,0,10); 
	abcorr_F = new TH2F("abcorr_F","abcorr_F",100,0,10,100,0,10);
	abcorr_L = new TH2F("abcorr_L","abcorr_L",100,0,10,100,0,10);
...
//	if ((*showerStart)[2] < 177.7) continue;
	...
	Double_t sH = HADshapeIL->GetRMS();
        Double_t sF = EMFshapeRL->GetRMS();
        Double_t sL = EMLshapeIL->GetRMS();
