...
//Get Shower starting point for each shower        
        ...
        HADshapeIL = new TH1F( (snameH.str()).c_str(),(snameH.str()).c_str() ,   30,0,30   );
...
//do not consider hits before shower starting points 
          ...
          HADshapeIL->Fill((h_H->GetXaxis()->GetBinCenter(b) - (*showerStart)[2])/hcalradlen, h_H->GetBinContent(b));
        }
         if (HADshapeIL->Integral()< energies[idx] * 0.5) continue;
...
//Now produce energy profile using estimated alpha and beta parameters
        std::stringstream name;
        name <<idx<<Nshower;
        funcH = new TF1("GammaH",GammaProfile,0  , 30,nGammaPar);
        funcH->SetParameters(pow(mH,2)/sH,mH/sH);
        Eprofile = new TH1F((name.str()).c_str(),(name.str()).c_str(),30,0,30);
...
// Now reproduce the shower profiles using the distribution of the alpha and beta parameters found in previous step from fullsim showers
    ...
    for (int i = 0; i < nsudo; ++i){
      ...
      std::stringstream name;
      name <<i;
      hists.push_back(RandomGammaProfile(exp(alpha) ,exp(beta), 30,30, (name.str()).c_str()));
    }
...
*/
    ...
    hists[hists.size()-1]->SetTitle("Average First Pi0 energy profile");
    hists[hists.size()-1]->GetXaxis()->SetTitle("Radiation length");
...
//Draw histograms
    can = new TCanvas("ft","ft",700,500);
    can->cd();
 hists[hists.size()-1]->SetMaximum(1.2*hists[hists.size()-1]->GetMaximum());
